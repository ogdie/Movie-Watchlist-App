...
import { getWatchedMovies, deleteMovie, editMovie } from "../services/api";
...
export default function WatchedMovies({ onEdit }) {
  ...
  useEffect(() => {
    async function loadMovies() {
      try {
        const data = await getWatchedMovies(); // traz apenas filmes com watched=true
        setMovies(data);
      } catch (error) {
      ...
      {movies.map(movie => (
        <div key={movie._id} className="bg-white/10 backdrop-blur-sm p-6 rounded-lg mb-4 border border-white/20 hover:bg-white/20 transition-all">
          <p style={{
            fontFamily: 'Bungee',
            textShadow: '1px 1px 2px rgba(0,0,0,0.3), -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000'
          }} className="text-xl font-semibold text-yellow-400">{movie.title} <span className="text-gray-300">({movie.year})</span></p>
          <p style={{
            fontFamily: 'Bungee',
            textShadow: '1px 1px 2px rgba(0,0,0,0.3), -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000'
          }} className="text-yellow-400 mt-2">Gênero: {movie.genre}</p>
          <p style={{
            fontFamily: 'Bungee',
            textShadow: '1px 1px 2px rgba(0,0,0,0.3), -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000'
